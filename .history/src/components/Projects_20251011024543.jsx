...
export default function Projects() {
  ...
  return (
    ...
    >
      ...
      <div className=" flex items-center justify-center w-[90vw] h-[90vh] mx-auto overflow-hidden">
        {/* Left arrow */}
        <button
          onClick={prevSlide}
          className="overflow-hidden absolute left-2 sm:left-6 text-white text-3xl sm:text-4xl z-10 bg-black/40 hover:bg-black/60 rounded-full p-2 transition"
        >
          ❮
        </button>
        ...
        <div
          ...
          style={{
            ...
            width: `${projects.length * 100}%`, // stretch track to fit all slides
          }}
        >
          {projects.map((project, i) => (
            ...
            >
              <div className="bg-white text-black rounded-2xl shadow-lg p-6 w-[100%] h-[100%] relative ">
                ...
                <p>
                  <strong>RESULTS:</strong> <br /> {project.results ?? "-"}
                </p>
              </div>
            </div>
          ))}
        ...
        <button
          onClick={nextSlide}
          className="overflow-hidden absolute right-2 sm:right-6 text-white text-3xl sm:text-4xl z-10 bg-black/40 hover:bg-black/60 rounded-full p-2 transition"
        >
          ❯
        </button>
      </div>
      ...
      <div className="flex gap-3 mt-8 overflow-hidden">
        {projects.map((_, i) => (
          <button
            key={i}
            onClick={() => setIndex(i)}
            className={`w-3 h-3 rounded-full transition-all overflow-hidden duration-300 ${
              i === index
                ? "bg-white scale-125"
