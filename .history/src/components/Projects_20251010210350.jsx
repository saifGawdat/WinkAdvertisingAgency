...
export default function Projects() {
  ...
  useEffect(() => {
    function measure() {
      ...
      const count = Math.max(1, Math.min(projects.length || 1, projects.length));
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (e.key === "ArrowRight") setIndex((i) => Math.min((projects.length || 1) - 1, i + 1));
    }
  ...
  useEffect(() => {
    ...
    function onEnd() {
      ...
      else if (moved < -threshold) setIndex((i) => Math.min((projects.length || 1) - 1, i + 1));
  ...
  if (loading)
    return (
      <section id="projects" className="w-screen min-h-screen projects py-12 bg-white text-black">
        <div className="container mx-auto px-4 text-center">Loading projects…</div>
      </section>
    );
  ...
  if (error)
    return (
      <section id="projects" className="w-screen min-h-screen projects py-12 bg-white text-black">
        <div className="container mx-auto px-4 text-center text-red-600">Error: {error}</div>
      </section>
    );
  ...
  if (!projects.length)
    return (
      <section id="projects" className="w-screen min-h-screen projects py-12 bg-white text-black">
        <div className="container mx-auto px-4 text-center">No projects found.</div>
      </section>
    );
  ...
  return (
    <section id="projects" className="w-screen min-h-screen projects py-12 bg-white text-black">
      ...
      <div ref={wrapperRef} className="w-full max-w-[1200px] mx-auto relative overflow-hidden" style={{ touchAction: "pan-y" }}>
        <div ref={sliderRef} style={sliderStyle}>
          {projects.map((project, idx) => {
            const logo = logos[idx % logos.length];
            return (
              <article key={project.id ?? idx} style={slideStyle} className="px-6 py-12 flex items-start justify-center">
                <div className="w-full max-w-[1100px]">
                  {/* logo top-left fixed 180x180 */}
                  <div className="relative bg-white rounded-lg shadow p-6" style={{ minHeight: 260 }}>
                    <div style={{ position: "absolute", top: 16, left: 16, width: 180, height: 180 }}>
                      <img
                        src={logo}
                    ...
                    <div style={{ marginTop: 220 }}>
                      <h3 className="text-3xl font-bold mb-4">{project.name ?? project.title ?? "Untitled"}</h3>

                      <p className="text-lg mb-4"><strong>BRIEF:</strong><br />{project.brief ?? project.description ?? "-"}</p>

                      <div className="mb-4">
                        <h4 className="text-xl font-semibold mb-2">Solutions</h4>
                        <p className="text-lg">{project.solution ?? project.solutions ?? "-"}</p>
                      </div>

                      <div>
                        <h4 className="text-xl font-semibold mb-2">Results</h4>
                        <p className="text-lg">{project.result ?? project.results ?? "-"}</p>
                      </div>
                    </div>
                  </div>
                </div>
