...
export default function Projects() {
  ...
  // stable prev/next
  function prev() {
    setIndex((i) => (projects.length ? (i <= 0 ? projects.length - 1 : i - 1) : 0));
  }
  function next() {
    setIndex((i) => (projects.length ? (i >= projects.length - 1 ? 0 : i + 1) : 0));
  }
  ...
  useEffect(() => {
    ...
    function onEnd() {
      ...
      const threshold = Math.min(120, (containerWidthRef.current || window.innerWidth) * 0.12); // adaptive threshold
  ...
  const visibleWidth = containerWidthRef.current || (typeof window !== "undefined" ? window.innerWidth : 0);
  ...
  const sliderStyle = {
    ...
    transition: isDragging ? "none" : "transform 0.45s cubic-bezier(.2,.9,.2,1)",
    display: "flex",
  };
  ...
  return (
    <section id="projects" className="projects w-screen min-h-screen bg-white text-black flex items-center justify-center py-12 px-4 overflow-hidden">
      <div ref={wrapperRef} className="wrapper w-full max-w-[1200px] rounded-[20px] shadow-lg bg-white relative overflow-hidden mx-auto">
        <div ref={sliderRef} style={sliderStyle} className="carousel">
          {projects.map((p, i) => {
            const logo = logos[i % logos.length];
            return (
              <article key={p.id || i} style={slideItemStyle} className="flex items-center justify-center px-6 py-12">
                <div className="w-full px-6 py-8">
                  <div className="max-w-[1100px] w-full mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                    ...
                    <div>
                      <h3 className="text-2xl md:text-3xl font-bold mb-3">{p.name || p.title}</h3>
                      <p className="text-sm md:text-base text-gray-800 mb-4">{p.brief || p.description || "Brief not provided."}</p>

                      <div className="mb-4">
                        <h4 className="font-semibold mb-2">Solutions</h4>
                        <p className="text-sm md:text-base text-gray-700">{p.solutions || "Solutions summary not provided."}</p>
                      </div>

                      <div>
                        <h4 className="font-semibold mb-2">Results</h4>
                        <p className="text-sm md:text-base text-gray-700">{p.results || p.kpis || "Results not provided."}</p>
                      </div>

                      {p.link && (
                        <a href={p.link} className="inline-block mt-6 bg-black text-white px-4 py-2 rounded-md" target="_blank" rel="noopener noreferrer">View case</a>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {/* controls */}
        <button onClick={prev} aria-label="Previous" className="absolute left-4 top-1/2 -translate-y-1/2 bg-black text-white p-3 rounded-full shadow-lg hidden md:block">‹</button>
        <button onClick={next} aria-label="Next" className="absolute right-4 top-1/2 -translate-y-1/2 bg-black text-white p-3 rounded-full shadow-lg hidden md:block">›</button>

        {/* dots */}
        <div className="flex gap-3 justify-center py-6">
          {projects.map((_, i) => (
            <button key={i} onClick={() => setIndex(i)} aria-label={`Go to slide ${i + 1}`} className={`w-3 h-3 rounded-full ${i === index ? "bg-black" : "bg-black/30"}`} />
          ))}
        </div>
      </div>
    </section>
