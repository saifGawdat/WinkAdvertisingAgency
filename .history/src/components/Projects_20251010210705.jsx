...
export default function Projects() {
  ...
  if (loading)
    return (
      <section
        id="projects"
        className="w-screen min-h-screen projects py-12 bg-white text-black flex items-center justify-center"
      >
  ...
  return (
    <section
      id="projects"
      className="w-screen min-h-screen projects py-12 bg-white text-black flex flex-col items-center justify-center"
    >
      <div className="w-full px-4">
        <h2 className="text-4xl font-bold mb-8 text-center text-white">Our Projects</h2>
      </div>

      <div
        ref={wrapperRef}
        className="w-full max-w-[960px] mx-auto relative overflow-hidden"
        style={{
          touchAction: "pan-y",
          ...
          {projects.map((_, i) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-transform duration-200 ${
                i === index ? "bg-black scale-125" : "bg-black/30 scale-100"
              }`}
            />
          ))}
        </div>
