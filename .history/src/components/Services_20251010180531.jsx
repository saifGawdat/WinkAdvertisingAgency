...
export default function Services() {
  // use public path (place branding.webp in public/assets/)
  const brandingImg = "/assets/branding.webp";
  ...
  // helper to normalize image paths (accepts absolute URLs, leading / public paths, or plain filenames)
  const normalizeSrc = (raw, fallback = "/assets/black-texture-projects.webp") => {
    if (!raw) return fallback;
    if (raw.startsWith("http") || raw.startsWith("/")) return raw;
    // if backend returns "assets/..." or "somefolder/file.webp" ensure leading slash
  ...
  return (
    <div className="services w-screen min-h-screen flex flex-col items-center justify-center bg-white px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-[1400px] mx-auto place-items-center">
        {serviceData.map((s) => {
          const imgSrc = normalizeSrc(s.img);
          return (
            <TiltedCard
              key={s.id}
              ...
              overlayContent={
                <div className="absolute inset-0 justify-center z-10 bg-transparent flex flex-col items-center gap-3 p-4">
                  <img
                    ...
                    alt="branding"
                    onError={(e) => {
                      // hide broken overlay image to remove alt icon
