...
export default function AboutUs() {
  ...
  const [aboutData, setAboutData] = useState([]); // Add state for data
  ...
  async function aboutFetch() {
    try {
      ...
      setAboutData(data); // Store data in state
    } catch (error) {
      console.log(error);
    }
  }
  ...
  useEffect(() => {
    aboutFetch(); // Call the fetch function when component mounts
            ...
            <details className="group">
              <summary className="flex justify-between items-center cursor-pointer list-none text-white p-8 bg-transparent border-b-2 border-white/20 animate-borderGlow text-2xl">
                <span className="font-semibold">
                  {aboutData.length > 0 ? aboutData[0].title : "Our Mission"}
                </span>
