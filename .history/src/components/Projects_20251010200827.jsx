...
export default function Projects() {
  ...
  const slideStyle = {
    ...
    transform,
    transition: draggingRef.current ? "none" : "transform 0.6s cubic-bezier(.2,.9,.2,1)",
    display: "flex",
    ...
    >
      <div className="wrapper w-full max-w-[1200px] rounded-[20px] shadow-lg bg-white relative overflow-hidden">
        <div
          ...
          onMouseLeave={() => { if (draggingRef.current) onPointerUp(); }}
        >
          {projects.map((p, i) => {
              ...
              >
                <div className="max-w-[1100px] w-full grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                  ...
                  <div className="px-2 md:px-6">
                    ...
                    {p.link && (
                      <a href={p.link} className="inline-block mt-6 bg-black text-white px-4 py-2 rounded-md" target="_blank" rel="noopener noreferrer">
                        View case
                      </a>
                    )}
                  </div>
        ...
        {/* controls */}
        <button onClick={prev} aria-label="Previous" className="absolute left-4 top-1/2 -translate-y-1/2 bg-black text-white p-3 rounded-full shadow-lg hidden md:block">
          ‹
        </button>
        <button onClick={next} aria-label="Next" className="absolute right-4 top-1/2 -translate-y-1/2 bg-black text-white p-3 rounded-full shadow-lg hidden md:block">
          ›
        </button>
