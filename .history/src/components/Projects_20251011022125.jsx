...
export default function Projects() {
  ...
  return (
    <section
      id="projects"
      className="projects py-24 flex flex-col items-center justify-center relative h-screen w-screen flex-wrap "
    >
      {projects.map((projects, index) => (
        <div key={projects.id} className="w-[50%] bg-white text-black">
          <img src={logos[index]} alt={projects.name} className="w-[180px] h-[180px]" />
          <p className="text-black">BRIEF: <br/>{projects.brief}</p>
          <p className="project-description">{projects.solution}</p>
          <p className="project-description">{projects.result}</p>
        </div>
      ))}
    </section>
  );
